"""Парсер Яндекс.Карт: ритуальные/флористические компании. Работает с РФ-IP.
...
CITIES = [
    ...
    "Мелитополь", "Бердянск", "Энергодар", "Токмак", "Васильевка", "Пологи",
    # Херсонская обл. (левобережье)
    "Геническ", "Новая Каховка", "Каховка", "Скадовск", "Алёшки",
]

# Ритуальные/флористические запросы. category → normalize() → client_type.
QUERIES = [
    ("ритуальные услуги {city}", "ритуальные услуги"),
    ("похоронное бюро {city}", "похоронное бюро"),
    ("ритуальный магазин {city}", "ритуальный магазин"),
    ("памятники надгробия {city}", "мастерская памятников"),
    ("венки ритуальные {city}", "ритуальный магазин"),
    ("организация похорон {city}", "ритуальные услуги"),
    ("благоустройство могил {city}", "кладбищенские услуги"),
    ("крематорий {city}", "крематорий"),
    ("цветочный магазин {city}", "цветочный магазин"),
]

EXTRA_QUERIES_GLOBAL = [
    ("ритуальные услуги Крым", "ритуальные услуги", "Крым"),
    ("памятники надгробия Крым", "мастерская памятников", "Крым"),
    ("ритуальные услуги Запорожская область", "ритуальные услуги", ""),
    ("ритуальные услуги Херсонская область", "ритуальные услуги", ""),
]
...
async def _process_query(context, query_text: str, category: str, city: str):
    ...
    try:
        ...
        added = 0
        for i, sn in enumerate(snippets[:50]):
            try:
                # имя из сниппета
                name = ""
                ...
                # уже в базе (этот или прошлый прогон)? Не тратим 5-8 сек
                # на клик и скрейп карточки организации.
                if dedup.is_seen(name, city):
                    continue
